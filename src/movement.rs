...
use crate::{Selected, Character, Marker, TILE_SIZE};

pub (crate) fn select_system(windows: Query<&Window>, cameras: Query<(&Camera, &GlobalTransform)>, characters: Query<(&Transform, Entity), (With<Character>, Without<Selected>)>, selected: Query<Entity, With<Selected>>, mut commands: Commands, buttons: Res<Input<MouseButton>>, mut materials: ResMut<Assets<ColorMaterial>>) {
...
pub (crate) fn move_system(mut selected: Query<&mut Transform, With<Selected>>, mut markers: Query<(&mut Transform, Entity), (With<Marker>, Without<Selected>)>, windows: Query<&Window>, cameras: Query<(&Camera, &GlobalTransform)>, buttons: Res<Input<MouseButton>>, mut commands: Commands, mut meshes: ResMut<Assets<Mesh>>, mut materials: ResMut<Assets<ColorMaterial>>) {
    let window = windows.single();
    let (camera, camera_transform) = cameras.single();
    ...
    if let Some(mut transform) = selected.iter_mut().next() {
        if let Some(mut marker) = markers.iter_mut().next() {
            if let Some(mouse_position) = window.cursor_position()
                .and_then(|cursor| camera.viewport_to_world(camera_transform, cursor))
                .map(|ray| ray.origin.truncate()) {
                marker.0.translation = Vec3::from((snap(mouse_position.x, 20, 1), snap(mouse_position.y, 20, 1), 0.0));
                if buttons.just_pressed(MouseButton::Left) {
...
fn snap(original: f32, numinator: u32, denuminator: u32) -> f32 {
    (original / numinator as f32 * denuminator as f32).round() * numinator as f32 / denuminator as f32
}
